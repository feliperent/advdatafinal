...
-- gold.dim_company (one row per ticker that exists in silver_prices_cleaned)
...
WHERE EXISTS (
    SELECT 1 FROM advdatafinal.silver.silver_prices_cleaned s
...
-- gold.dim_chunk (UNION of 10-K and 8-K chunked with filing_type tag)
CREATE OR REFRESH MATERIALIZED VIEW advdatafinal.gold.dim_chunk AS
SELECT '10K' AS filing_type, chunk_key, accession, symbol, company_key, filing_date,
       chunk_index, n_tokens, body_chunk, embedding
FROM advdatafinal.silver.silver_filings_10k_chunked
UNION ALL
SELECT '8K' AS filing_type, chunk_key, accession, symbol, company_key, filing_date,
       chunk_index, n_tokens, body_chunk, embedding
FROM advdatafinal.silver.silver_filings_8k_chunked;

-- gold.fct_sentiment_per_day (per-day mean FinBERT + article counts)
CREATE OR REFRESH MATERIALIZED VIEW advdatafinal.gold.fct_sentiment_per_day AS
WITH news_daily AS (
    SELECT symbol, to_date(published_at) AS trade_date,
           AVG(finbert_score) AS news_score, COUNT(*) AS n_news
    FROM advdatafinal.silver.silver_news_scored
    GROUP BY symbol, to_date(published_at)
),
press_daily AS (
    SELECT symbol, to_date(published_at) AS trade_date,
...
SELECT
    md5(coalesce(n.symbol, p.symbol))                                AS company_key,
    md5(cast(coalesce(n.trade_date, p.trade_date) as STRING))        AS date_key,
...
-- gold.fct_feature_panel_daily_full (15 structured + 4 sentiment + 5 PCA = 24 features)
CREATE OR REFRESH MATERIALIZED VIEW advdatafinal.gold.fct_feature_panel_daily_full AS
WITH pca_asof AS (
    SELECT p.symbol, p.trade_date,
           e.filing_pc1, e.filing_pc2, e.filing_pc3, e.filing_pc4, e.filing_pc5,
           ROW_NUMBER() OVER (
               PARTITION BY p.symbol, p.trade_date
               ORDER BY e.filing_date DESC NULLS LAST
           ) AS rn
