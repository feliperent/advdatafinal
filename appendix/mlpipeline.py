...
#   raw -> silver -> gold dims + sentiment + feature_panel + feature_panel_full + embedding + chunk
# This notebook does the only two objects DLT cannot:
#   - XGBoost training (weights are not derivable from one read)
#   - Walk-forward backtest (depends on trained predictions)
# Writes gold.fct_predictions and gold.fct_backtest_pnl_daily as plain Delta tables.
...
# Walk-forward fold generator (3y train / 5d gap / 63d test, quarterly advance)
...
print(f"Rung 1: {len(preds_rung1)} preds")

# COMMAND ----------

# Rung 2 | 24 features (15 structured + 4 sentiment + 5 PCA)
preds_rung2 = []
for fold in folds():
    train = panel[(panel["trade_date"] >= pd.Timestamp(fold.train_start)) & (panel["trade_date"] <= pd.Timestamp(fold.train_end))]
    test  = panel[(panel["trade_date"] >= pd.Timestamp(fold.test_start))  & (panel["trade_date"] <= pd.Timestamp(fold.test_end))]
...
print(f"Rung 2: {len(preds_rung2)} preds")
